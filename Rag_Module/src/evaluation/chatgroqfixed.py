...
class GroqChatFixed(ChatGroq):
    ...
    def _create_chat_result(self, response, *args, **kwargs):
        # Forward ALL extra args (langchain-groq>=0.3 passes `params` as the 2nd
        # positional arg). The older override dropped it -> TypeError "missing
        # required positional argument: 'params'". The parent now populates
        # generation_info from params itself, so we just delegate.
