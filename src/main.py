...
async def startup_span():
    ...
    # ── Embedding client ──────────────────────────────────────────────
    # WHY separate from generation_client?
    # Embedding uses a local HuggingFace model (fast, free, offline).
    # Generation uses Groq API (powerful, cloud).
    # They're different backends so we create them separately.
    ...
    # ── Reranker (optional) ───────────────────────────────────────────
    # WHY optional? RERANKER_ENABLED=False means pipeline runs without
    # reranking — useful for baseline evaluation before/after comparison.
    # When enabled, cross-encoder reranks top-K vector search candidates
