...
concept_mapper.py
=================
Maps extracted concepts (from `concepts` table) to canonical nodes in
the knowledge graph (`knowledge_nodes`) using a three-stage pipeline:

  Stage 1 — Exact lookup
      Normalised name and alias string matching. Zero model cost.

  Stage 2 — Type-scoped bi-encoder retrieval
      FAISS index queried — same-type nodes first, global fallback.
      Type-scoping reduces false positives (e.g. "heap" data structure
      vs "heap" memory region).

  Stage 3 — Cross-encoder reranking
      (concept text, candidate text) pair scored by a cross-encoder.
      Best candidate above MATCH_THRESHOLD stored as the mapping.

Schema note:
    concepts.id              — VARCHAR (UUID)
    knowledge_nodes.id       — INTEGER (serial)
    concept_node_mappings:
        concept_id           — VARCHAR (UUID) FK → concepts
        node_id              — INTEGER        FK → knowledge_nodes

Usage:
    python concept_mapper.py --course-id <uuid>
...
# ---------------------------------------------------------------------------
# Embedding cache
# node_ids stored as int32 — knowledge_nodes.id is INTEGER
# ---------------------------------------------------------------------------
