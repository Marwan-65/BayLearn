...
from ...formatting import expr_text, format_matrix_steps, matrix_to_latex
from ...models.requests import SolverRequest
from ...utils.constants import VALID_MATRIX_OPERATIONS


def handle_matrix_operation(request: SolverRequest) -> str:
    ...
    try:
        ...
        if operation == "determinant":
            result = matrix.det()
            step_text = format_matrix_steps(matrix, operation, result)
            return f"{step_text}\n\nFinal Result: Determinant = {expr_text(result)}"

        if operation == "inverse":
            det_value = sp.simplify(matrix.det())
            if det_value == 0:
                step_text = format_matrix_steps(matrix, operation, None)
        ...
        if operation == "eigenvalues":
            ...
            step_text = format_matrix_steps(matrix, operation, result)
            eigenval_text = ", ".join(
                f"λ{i} = {expr_text(val)} (multiplicity {mult})"
        ...
        if operation == "eigenvectors":
            ...
            for val, mult, vecs in result:
                eigenvec_parts.append(f"\nEigenvalue λ = {expr_text(val)} (multiplicity {mult})")
                eigenvec_parts.append("Eigenvector(s):")
                for vec in vecs:
                    vec_latex = matrix_to_latex(vec)
        ...
        if operation == "trace":
            result = matrix.trace()
            step_text = format_matrix_steps(matrix, operation, result)
            return f"{step_text}\n\nFinal Result: Trace = {expr_text(result)}"
