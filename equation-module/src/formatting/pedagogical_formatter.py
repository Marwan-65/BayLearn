...
def explain_general_math_steps(operation: str, sympy_equations: List[sp.Eq], target_vars: List[sp.Symbol], result: Any) -> str:
    """format generic explanation steps
    Args:
    ...
    if operation == "derive":
        ...
        steps.append(f"  Expression: ${convert_to_safe_latex(expression)}$")
        ...
        if len(dependent_factors) >= 2:
            ...
            steps.append(f"  Here, $f = {convert_to_safe_latex(f_part)}$ and $g = {convert_to_safe_latex(g_part)}$")
        ...
        if expression.func in {
            ...
            if variable in inner.free_symbols and sp.diff(inner, variable) != 1:
                ...
                steps.append(f"  Inner function: $g({variable}) = {convert_to_safe_latex(inner)}$")

        unsimplified = sp.diff(expression, variable, evaluate=False)
        steps.append("Step 3: Apply the rule formula")
        steps.append(f"  $\\frac{{d}}{{d{variable}}}\\left({convert_to_safe_latex(expression)}\\right) = {convert_to_safe_latex(unsimplified)}$")
        simplified = sp.simplify(result)
        if sp.simplify(unsimplified - simplified) != 0:
            steps.append("Step 4: Simplify")
            steps.append(f"  ${convert_to_safe_latex(unsimplified)} = {convert_to_safe_latex(simplified)}$")
        else:
            steps.append("Step 4: Final derivative")
            steps.append(f"  ${convert_to_safe_latex(simplified)}$")
        return "\n".join(steps)

    if operation in ["solve", "solve_system"]:
        for index, equation in enumerate(sympy_equations, start=1):
            steps.append(f"  Equation {index}: ${convert_to_safe_latex(equation.lhs)} = {convert_to_safe_latex(equation.rhs)}$")
        ...
        for index, equation in enumerate(sympy_equations, start=1):
            ...
            steps.append(f"  Eq{index}: ${convert_to_safe_latex(standard_form)} = 0$")
        ...
        if isinstance(result, dict):
            for variable in target_vars:
                if variable in result:
                    steps.append(f"  ${variable} = {convert_to_safe_latex(result[variable])}$")
                ...
                for solution_index, solution in enumerate(result, start=1):
                    ...
                    for variable in target_vars:
                        if variable in solution:
                            pieces.append(f"{variable} = {convert_to_safe_latex(solution[variable])}")
                    steps.append(f"  Solution {solution_index}: {', '.join(pieces)}")
            else:
                if len(target_vars) == 1:
                    variable = target_vars[0]
                    for solution_index, value in enumerate(result, start=1):
                        steps.append(f"  Solution {solution_index}: ${variable} = {convert_to_safe_latex(value)}$")
                else:
                    steps.append(f"  Solutions: {[convert_to_safe_latex(v) for v in result]}")
        else:
            steps.append(f"  Solution: ${convert_to_safe_latex(result)}$")
        ...
        steps.append(f"  $\\int {convert_to_safe_latex(expression)} \\, d{variable}$")
        steps.append("Step 3: Final antiderivative")
        steps.append(f"  ${convert_to_safe_latex(result)} + C$")

    return "\n".join(steps)


def check_if_equation_is_linear(equation: sp.Eq, variables: List[sp.Symbol]) -> bool:
    """decide if equation is linear from given variables."""
...
def explain_linear_equation_steps(
    ...
    Returns:
        Step text when linear case applies else None
    """
    if not sympy_equations or not target_vars:
        return None
    if not all(check_if_equation_is_linear(equation, target_vars) for equation in sympy_equations):
        return None
    ...
    for index, expression in enumerate(standard_forms, start=1):
        steps.append(f"  Eq{index}: ${convert_to_safe_latex(expression)} = 0$")
    ...
    if len(sympy_equations) == 1 and len(target_vars) == 1:
        ...
        steps.append(f"  ${convert_to_safe_latex(coefficient)} \\cdot {variable} + ({convert_to_safe_latex(constant)}) = 0$")
        steps.append(f"  ${convert_to_safe_latex(coefficient)} \\cdot {variable} = -{convert_to_safe_latex(constant)}$")
        isolated = sp.simplify(-constant / coefficient)
        steps.append(f"  ${variable} = {convert_to_safe_latex(isolated)}$")
        steps.append("Step 3: Final answer")
        if isinstance(result, list):
            for solution_index, value in enumerate(result, start=1):
                steps.append(f"  Solution {solution_index}: ${variable} = {convert_to_safe_latex(value)}$")
        else:
            steps.append(f"  ${variable} = {convert_to_safe_latex(result)}$")
        return "\n".join(steps)
    ...
    if len(sympy_equations) == 2 and len(target_vars) == 2:
        ...
        steps.append(f"  Eq1: $({convert_to_safe_latex(a11)}){x_var} + ({convert_to_safe_latex(a12)}){y_var} = {convert_to_safe_latex(c1)}$")
        steps.append(f"  Eq2: $({convert_to_safe_latex(a21)}){x_var} + ({convert_to_safe_latex(a22)}){y_var} = {convert_to_safe_latex(c2)}$")
        ...
        steps.append(
            f"  Determinant = ${convert_to_safe_latex(a11)} \\cdot {convert_to_safe_latex(a22)} - "
            f"{convert_to_safe_latex(a21)} \\cdot {convert_to_safe_latex(a12)} = {convert_to_safe_latex(determinant)}$"
        )
        x_num = sp.simplify(c1 * a22 - c2 * a12)
        steps.append(f"  ${x_var} = \\frac{{{convert_to_safe_latex(x_num)}}}{{{convert_to_safe_latex(determinant)}}}$")
        x_value = sp.simplify(x_num / determinant)
        steps.append(f"  ${x_var} = {convert_to_safe_latex(x_value)}$")
        ...
        if a12 != 0:
            ...
            steps.append(
                f"  From Eq1: $({convert_to_safe_latex(a11)}) \\cdot ({convert_to_safe_latex(x_value)}) + "
                f"({convert_to_safe_latex(a12)}){y_var} = {convert_to_safe_latex(c1)}$"
            )
        elif a22 != 0:
            y_num = sp.simplify(c2 - a21 * x_value)
            y_value = sp.simplify(y_num / a22)
            steps.append(
                f"  From Eq2: $({convert_to_safe_latex(a21)}) \\cdot ({convert_to_safe_latex(x_value)}) + "
                f"({convert_to_safe_latex(a22)}){y_var} = {convert_to_safe_latex(c2)}$"
            )
        else:
            return None
        steps.append(f"  ${y_var} = {convert_to_safe_latex(y_value)}$")

        steps.append("Step 5: Final answer")
        if isinstance(result, dict):
            steps.append(f"  ${x_var} = {convert_to_safe_latex(result.get(x_var, x_value))}$")
            steps.append(f"  ${y_var} = {convert_to_safe_latex(result.get(y_var, y_value))}$")
        elif isinstance(result, list) and result and isinstance(result[0], dict):
            for solution_index, solution in enumerate(result, start=1):
                parts = []
                for variable in target_vars:
                    if variable in solution:
                        parts.append(f"{variable} = {convert_to_safe_latex(solution[variable])}")
...
def explain_differential_equation_steps(diff_eq: sp.Eq, solution: Any, dep_var_name: str) -> str:
    """format differential equation with explanation."""
    steps: List[str] = []
    ...
    eq_latex = convert_to_safe_latex(lhs - rhs) if rhs != 0 else convert_to_safe_latex(lhs)
    ...
    solution_latex = convert_to_safe_latex(solution.rhs)
...
def explain_matrix_operation_steps(matrix: Any, operation: str, result: Any) -> str:
    """Format explanation for matrix operations."""
    steps: List[str] = []
    steps.append("Step 1: Parse and Display Matrix")
    steps.append("  Input matrix:")
    matrix_latex = convert_to_safe_latex(matrix)
    ...
    if operation == "determinant":
        if hasattr(matrix, "rows") and hasattr(matrix, "cols") and matrix.rows == 2 and matrix.cols == 2:
            ...
            steps.append(
                f"  $\\det(A)=({convert_to_safe_latex(a)}\\cdot {convert_to_safe_latex(d)})-({convert_to_safe_latex(b)}\\cdot {convert_to_safe_latex(c)})$"
            )
            steps.append(f"  $= {convert_to_safe_latex(ad)} - {convert_to_safe_latex(bc)} = {convert_to_safe_latex(result)}$")
        else:
            ...
            steps.append(f"  $\\det(A) = {convert_to_safe_latex(result)}$")
        ...
        if hasattr(matrix, "rows") and hasattr(matrix, "cols") and matrix.rows == 2 and matrix.cols == 2:
            ...
            steps.append(f"  $\\det(A)=({convert_to_safe_latex(a)}\\cdot {convert_to_safe_latex(d)})-({convert_to_safe_latex(b)}\\cdot {convert_to_safe_latex(c)})$")
            steps.append(f"  $= {convert_to_safe_latex(ad)}-{convert_to_safe_latex(bc)} = {convert_to_safe_latex(det_a)}$")
        else:
            steps.append("Step 3: Compute determinant to verify invertibility")
            steps.append(f"  $\\det(A) = {convert_to_safe_latex(det_a)}$")
        ...
        steps.append("Step 4: Verify condition")
        steps.append(f"  Since $\\det(A)={convert_to_safe_latex(det_a)} \\neq 0$, inverse exists.")
        steps.append("Step 5: Compute inverse")
        steps.append(f"  $A^{{-1}} = {convert_to_safe_latex(result)}$")
        ...
        steps.append(f"  ${convert_to_safe_latex(char_poly.as_expr())} = 0$")
        steps.append("Step 4: Solve for roots")
        if isinstance(result, dict) and result:
            for idx, (eigenvalue, multiplicity) in enumerate(result.items(), start=1):
                steps.append(f"  $\\lambda_{idx} = {convert_to_safe_latex(eigenvalue)}$ (multiplicity {multiplicity})")
        else:
        ...
        if isinstance(result, list) and result:
            for idx, triple in enumerate(result, start=1):
                ...
                steps.append(f"  Eigenvalue {idx}: $\\lambda = {convert_to_safe_latex(eigenvalue)}$ (multiplicity {multiplicity})")
                for vec_idx, vector in enumerate(vectors, start=1):
                    steps.append(f"  Basis vector {vec_idx}: ${convert_to_safe_latex(vector)}$")
        else:
        ...
        steps.append(f"  $A^T = {convert_to_safe_latex(result)}$")
        ...
        steps.append(f"  $\\operatorname{{RREF}}(A) = {convert_to_safe_latex(result)}$")
        ...
        steps.append(f"  $\\operatorname{{RREF}}(A) = {convert_to_safe_latex(rref_matrix)}$")
        steps.append("Step 4: Count pivot columns")
        steps.append(f"  Pivot columns: {pivots}")
        steps.append(f"  $\\operatorname{{rank}}(A) = {convert_to_safe_latex(result)}$")
        ...
        if hasattr(matrix, "rows") and hasattr(matrix, "cols"):
            ...
            diagonal_sum = " + ".join(convert_to_safe_latex(v) for v in diagonal) if diagonal else "0"
            steps.append("Step 3: Substitute diagonal entries")
            steps.append(f"  $\\operatorname{{tr}}(A) = {diagonal_sum} = {convert_to_safe_latex(result)}$")
        else:
            steps.append("Step 3: Compute trace")
            steps.append(f"  $\\operatorname{{tr}}(A) = {convert_to_safe_latex(result)}$")

    else:
        steps.append("Step 3: Complete the operation")
        steps.append(f"  Result: ${convert_to_safe_latex(result)}$")

    return "\n".join(steps)


def explain_limit_evaluation_steps(expression: sp.Expr, variable: sp.Symbol, approach_value: Any, direction: str, result: Any) -> str:
    """Format steps for limits."""
    steps: List[str] = []
    steps.append("Step 1: Identify Limit Problem")
    steps.append(f"  Expression: ${convert_to_safe_latex(expression)}$")
    steps.append(f"  Variable: {variable}")
    steps.append(f"  Approaching: ${convert_to_safe_latex(approach_value)}$")
    if direction == "+":
...
def explain_taylor_series_steps(expression: sp.Expr, variable: sp.Symbol, point: int, order: int, result: Any) -> str:
    """Format educational steps for Taylor/Maclaurin series."""
    steps: List[str] = []
    steps.append("Step 1: Identify Series Expansion")
    steps.append(f"  Expression: ${convert_to_safe_latex(expression)}$")
    steps.append(f"  Variable: {variable}")
    steps.append(f"  Expansion point (a): ${convert_to_safe_latex(point)}$")
    steps.append(f"  Order: {order} terms")
    if point == 0:
        steps.append("\n  This is a Maclaurin series (Taylor series at $x=0$)")
    else:
        steps.append(f"\n  This is a Taylor series centered at $x={convert_to_safe_latex(point)}$")
    steps.append("\nStep 2: Apply Taylor Formula")
    steps.append("  $f(x) = f(a) + f'(a)(x-a) + \\frac{f''(a)}{2!}(x-a)^2 + \\frac{f'''(a)}{3!}(x-a)^3 + \\ldots$")
    steps.append(f"  Compute derivatives at $x={convert_to_safe_latex(point)}$ up to order {order}")
    steps.append("\nStep 3: Series Expansion")
    steps.append(f"  ${convert_to_safe_latex(result)} + O({variable}^{{{order+1}}})$")
    return "\n".join(steps)


def explain_partial_derivative_steps(expression: sp.Expr, variables: List[sp.Symbol], result: Any) -> str:
    ...
    for i, var in enumerate(variables, 1):
        ...
        steps.append(f"  Result: ${convert_to_safe_latex(temp_expr)}$")
    steps.append("\nStep 3: Final Partial Derivative")
    steps.append(f"  ${convert_to_safe_latex(result)}$")
    return "\n".join(steps)
