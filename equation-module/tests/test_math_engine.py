...
from baylearn.math_engine.formatting import format_final_math_answer
from baylearn.math_engine.models.requests import EquationData, SolverRequest
from baylearn.math_engine.parsing.parser_utils import extract_sympy_equations_from_payload, extract_sympy_symbols_from_payload
from baylearn.math_engine.parsing.validators import validate_math_request_structure
from baylearn.math_engine.solver import dispatcher, orchestrator
from baylearn.math_engine.solver.operations.algebra import solve_algebraic_equation
from baylearn.math_engine.solver.operations.calculus import calculate_derivative, calculate_integral
from baylearn.math_engine.solver.operations.differential_equations import solve_ode
from baylearn.math_engine.solver.operations.limits import evaluate_limit
from baylearn.math_engine.solver.operations.matrices import perform_matrix_operation
from baylearn.math_engine.solver.operations.partial_derivatives import calculate_partial_derivative
from baylearn.math_engine.solver.operations.series import expand_taylor_series
from baylearn.math_engine.solver.operations.simplification import simplify_algebraic_expression
...
def test_request_normalization_and_parsing():
    ...
    equations = extract_sympy_equations_from_payload(request)
    variables = extract_sympy_symbols_from_payload(request)
    assert equations == [sp.Eq(sp.Symbol("x") + 2, 0)]
    assert variables == [sp.Symbol("x")]


def test_validate_math_request_structure_enforces_core_rules():
    with_exception = [
    ...
    for request in with_exception:
        try:
            validate_math_request_structure(request)
    ...
    validate_math_request_structure(
        SolverRequest(
            operation="unknown",
            equations=[],
            target_variables=[],
...
def test_solve_algebraic_equation_returns_full_result():
    output = solve_algebraic_equation(make_request("solve", "x - 2", "0", ["x"]))
...
def test_derivative_and_integral_handlers_include_graphable_sections():
    derivative_output = calculate_derivative(make_request("derive", "x**2", "0", ["x"]))
    integral_output = calculate_integral(make_request("integrate", "x", "0", ["x"]))
...
def test_limit_series_simplify_and_partial_derivative_handlers_work():
    limit_output = evaluate_limit(
        make_request("limit", "(x**2 - 1)/(x - 1)", "1", ["x"], direction="+-")
    )
    series_output = expand_taylor_series(
        make_request("series", "exp(x)", "0", ["x"], point=0, order=4)
    )
    simplify_output = simplify_algebraic_expression(make_request("simplify", "(x**2 - 1)/(x - 1)", "0", ["x"]))
    partial_output = calculate_partial_derivative(
...
def test_matrix_and_differential_equation_handlers_work():
    determinant_output = perform_matrix_operation(
        SolverRequest(
            operation="matrix_ops",
            equations=[EquationData(lhs="[[1,2],[3,4]]", rhs="0")],
            target_variables=[],
...
def test_orchestrator_and_public_wrappers_cover_the_full_backend_flow(monkeypatch):
    ...
    monkeypatch.setattr(orchestrator, "translate_natural_language_to_math_json", lambda _: payload)
